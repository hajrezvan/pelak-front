...
const getCachedProduct = cache(async (locale: string, articles: string) => {

  const baseUrl = process.env.API_BASE_URL;
  const url = baseUrl + "/categories_slug_v0101?type=eq.a&lang=eq."+locale+"&active=eq.true&categorySlug=eq."+articles.toLowerCase().replace(/%20/g, "-");
...
export default async function ArticlesPage({
  ...
  params: Promise<{ locale: string, articles: string }>;
}) {

  const { locale, articles } = await params;

  const PDcategory: PIcategory[] = await getCachedProduct(locale,articles);
        ...
        <Image
          ...
          <div className="absolute bottom-0 px-P-4 text-P-TextWhite md:px-P-5 w-full text-center">
            <h1 className="text-P-PrimaryDark text-3xl font-bold tracking-tight md:text-5xl lg:text-6xl text-shadow-P-Background text-shadow-sm">
              {PDcategory[0].category || articles}
            </h1>
          </div>
        </div>
